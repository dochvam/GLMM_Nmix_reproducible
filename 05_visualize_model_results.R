...
#### Fig 1. Plot chosen models by AIC ####
...
daic_colors <- daic_dat_to_plot %>%
  ...
  theme_void() +
  geom_hline(yintercept = 0) +
  xlab(expression("Species-subregion dataset, sorted by "*Delta*"AIC")) + 
  ylab("DELETE ME") + 
  labs(col = "Model", fill = "Model") +
  geom_vline(xintercept = x_intercepts, col = "#474747", size = 1) +
  scale_color_manual(values = model_line_colors) +
...
grid.arrange(textGrob("(a)", 0.1, just = "bottom"),
  ...
  ggsave(filename = "output/plots/Fig1_resub.pdf",
...
spec_choice_plot <- wide_om_df %>% 
  ...
  theme(panel.grid.major.x = element_blank(), 
        ...
        panel.grid.minor.x = element_blank()) +
  coord_flip()
ggsave("output/plots/FigS3_spec_choice.jpg", device = "jpeg", spec_choice_plot,
       width = 6, height = 4)


#### Fig 2. Trends in selection by complexity ####
...
(aodf_w_abd %>% 
    ...
    theme(panel.grid.major.x = element_blank(),
          panel.grid.minor.x = element_blank())) %>% 
  ggsave(filename = "output/plots/Fig2_resub.pdf",
         width = 6.5, height = 4)
...
arrangeGrob(grobs = list(
  ...
  ggsave(filename = "output/plots/FigS2_fAIC.jpg", width = 9, height = 6)



#### Fig S1. Plot subregion buffers ####
library(rgdal)
